...
func firstCheck(permutation [][]byte, conn net.Conn, digit byte) ([][]byte,[]int) {
...
func check(conn net.Conn, selectedArray [][]byte, digit byte, firstCountResult[]int) [][]byte {
	...
	for h:=0; h<100; h++{  
		...
		if h%2 == 1 {
			...
			fmt.Println(selectedArray,"selectedArrayのもとのやつ")
			fmt.Println(selectedArray3,"完全一致")
			fmt.Println(mostlyCorrect,"ほぼ一致")
			fmt.Println(partlyCorrect,"部分一致")
			selectedArray3 = append(selectedArray3, mostlyCorrect...)
